...
if (!isset($_SESSION["user_id"])) echo "<script>window.location.href='./login_sys/login.php'</script>";
...
<body class="theme-set disable-scroll">
    ...
    <section class="enquiry form-add">
        <div class="container" style="max-width: 90vh;">
            <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
                ...
                <!--Date picker -->
                <div data-mdb-input-init class="form-outline mb-4">
                    <input type="date" class="form-control" id="datepicker" name="event-date">
                    <label class="form-label" for="datepicker">Event Date</label>
                </div>

                <!--Time picker -->
                <div data-mdb-input-init class="form-outline mb-4">
                    <input type="time" class="form-control" id="datepicker" name="event-time">
                    <label class="form-label" for="datepicker">Event Time</label>
                </div>

                <div data-mdb-input-init class="form-outline mb-4">
                    <select class="form-select mb-4" name="eventList" aria-label="Default select" style="background-color: transparent;">
                ...
                <div class="alert alert-success alert-dismissible fade" role="alert" id="success-msg" style="background-color: #04210f; color: #14a44d;">
                    ...
                    <div class="msg-box" style="margin-left: 10px;" >
                        <strong>Success!</strong> The gallery successfuly created.
                ...
                <!-- Submit button -->
                <button type="submit" class="btn btn-block mb-4 text-white" style="background-color: #ff4500;">Book</button>
            </form>
        </div>
    </section>
    ...
    if ($_SERVER["REQUEST_METHOD"] == 'POST') {
        if (isset($_POST['address'], $_POST['msg'], $_POST['event-date'], $_POST['event-time']) && !empty($_POST['eventList']) && IsEmpty(array($_POST['address'], $_POST['msg'], $_POST['event-date'], $_POST['event-time']))) {
            ...
            $event_list = $_POST['eventList'];
            $user_id = $_SESSION['user_id'];
            

            $query_insert_enqurey = "INSERT INTO enquiry( enquiry_id, customer_id, event_type, event_date, event_time, event_location, message ) VALUES ( '$enqurey_id', '$user_id', '$event_list', '$event_date', '$event_time', '$location', '$msg' )";
        
            if ($conn -> query($query_insert_enqurey) == true) {
