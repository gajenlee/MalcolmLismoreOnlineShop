...
<body class="theme-set disable-scroll">
        ...
        <div class="p-5 bg-body-tertiary mb-4" style="background-color: transparent !important;">
            ...
            <nav class="d-flex">
                <h6 class="mb-0">
                    ...
                    <span>/</span>
                    <a href="./update-image.php" class="text-reset"><u>Image</u></a>
                </h6>
            </nav>
        </div>
    ...
    <?php
    $image_id = $_GET["id"];
    $image_select_query = "SELECT * FROM images INNER JOIN gallery ON images.gallery_id = gallery.gallery_id WHERE image_id='$image_id'";
    $result = mysqli_query($conn, $image_select_query);
    $row = mysqli_fetch_assoc($result);
    ?>

    <section class="form-add update-images">
        <div class="container">
            <form action="" method="post" class="myForm">
                <div class="form-outline" data-mdb-input-init>
                    <input type="text" value="<?php echo $row["image_name"]; ?>" name="imageName" id="imageName" class="form-control" data-mdb-showcounter="true" maxlength="25" />
                    <label class="form-label" for="imageName">Image Name</label>
                    <div class="form-helper"></div>
                </div>
                <div class="form-outline" data-mdb-input-init>
                    <textarea class="form-control" name="imageDes" id="imageDes" rows="4"><?php echo $row["image_description"]; ?></textarea>
                    <label class="form-label" for="imageDes">Image Description</label>
                </div>
                <div class="form-outline" data-mdb-input-init>
                    <input type="file" name="filePath" class="form-control" id="customFile" />
                </div>
                <div class="form-outline form-control" style="background-color: transparent; padding: 10px;" data-mdb-input-init>
                    <h6 id="content_changer" style="margin: 0;"><?php echo $row["image_path"]; ?></h5>
                </div>
                <div class="form-outline" data-mdb-input-init>
                    <select class="form-select"  name="galleryList" aria-label="Default select" style="background-color: transparent;">
                        <option selected>Select a Gallery</option>
                        <?php

                        $gallery_select_query = "SELECT * FROM gallery";
                        $result_gallery = mysqli_query($conn, $gallery_select_query);
                        while ($rows_gallery = mysqli_fetch_assoc($result_gallery)) {

                            if ($rows_gallery['gallery_id'] == $row['gallery_id']) { ?>
                                <option selected value="<?php echo $rows_gallery['gallery_id']; ?>"><?php echo $rows_gallery['gallery_name']; ?></option>
                            <?php } else { ?>
                                <option value="<?php echo $rows_gallery['gallery_id']; ?>"><?php echo $rows_gallery['gallery_name']; ?></option>
                            <?php } ?>
                        <?php } ?>
                    </select>

                    <script>
                        const select_id = document.querySelector("#customFile");
                        const content_changer = document.querySelector("#content_changer");
                        select_id.addEventListener('change', (e) => {
                            path_split = select_id.value.split('\\');
                            content_changer.innerHTML =   "/assets/imgs/malcom-photos/" + path_split[path_split.length -1];
                        })
                    </script>
                </div>
                ...
                <div class="form-outline control-btns">
                    <button type="submit" class="btn text-white" data-mdb-ripple-init style="background-color: #ff4500;" name="submit" role="button">
                        <i class="fa fa-plus" aria-hidden="true"></i>
                        Update
                    </button>
                    <a class="btn text-white" data-mdb-ripple-init style="background-color: #ff4500;" href="./show-image.php" role="button">
                        Show image Table
                    </a>
                </div>
            </form>
    ...
    if (isset($_POST["submit"])) {
        ...
        if (isset($_POST['imageName'], $_POST['imageDes'], $_POST['filePath'], $_POST['galleryList'])) {

            $image_name = validate_inputs($_POST['imageName']);
            $image_des = $_POST['imageDes'];
            $image_filePath = $basePath . $_POST['filePath'];
            $gallery_id = $_POST['galleryList'];


            $image_update_query = "UPDATE images SET image_name='$image_name', image_path='$image_filePath', image_description='$image_des', image_gallery_id='$image_gallery_id' WHERE image_id='$image_id'";
            
            mysqli_query($conn, $image_update_query);
            if ($conn -> query($image_update_query) == true) echo "<script> window.location.href='./show-image.php'; </script>";
