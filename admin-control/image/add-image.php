...
<body class="theme-set disable-scroll">
        ...
        <div class="p-5 bg-body-tertiary mb-4" style="background-color: transparent !important;">
            ...
            <nav class="d-flex">
                <h6 class="mb-0">
                    ...
                    <span>/</span>
                    <a href="./add-image.php" class="text-reset"><u>Image</u></a>
                </h6>
            </nav>
        </div>
    ...
    <section class="form-add add-images">
        <div class="container">
            <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
                <div class="form-outline" data-mdb-input-init>
                    <input name="imageId" style="background-color: transparent;" class="form-control" id="imageId" type="text" value="<?php echo uniqid(); ?>" readonly />
                    <label class="form-label" for="imageId">Image ID</label>
                </div>
                <div class="form-outline" data-mdb-input-init>
                    <input type="text" name="imageName" id="imageName" class="form-control" data-mdb-showcounter="true" maxlength="50" />
                    <label class="form-label" for="imageName">Image Name</label>
                    <div class="form-helper"></div>
                </div>
                <div class="form-outline" data-mdb-input-init>
                    <textarea class="form-control" name="imageDes" id="imageDes" rows="4"></textarea>
                    <label class="form-label" for="imageDes">Image Description</label>
                </div>
                <div class="form-outline" data-mdb-input-init>
                    <input type="file" class="form-control" name="filePath" id="customFile" />
                </div>
                <div class="form-outline" data-mdb-input-init>
                    <select name="galleryList" class="form-select" aria-label="Default select" style="background-color: transparent;">
                        ...
                        ?>
                            <option value="<?php echo $rows['gallery_id']; ?>"><?php echo $rows['gallery_name']; ?></option>
                        <?php } ?>
                    </select>
                </div>
                ...
                <div class="alert alert-success alert-dismissible fade" role="alert" id="success-msg" style="background-color: #04210f; color: #14a44d;">
                    ...
                    <div class="msg-box" style="margin-left: 10px;" >
                        <strong>Success!</strong> The image successfuly add to the gallery.
                ...
                <div class="form-outline control-btns">
                    <button type="submit" class="btn text-white" data-mdb-ripple-init style="background-color: #ff4500;" role="button">
    ...
    if ($_SERVER["REQUEST_METHOD"] == 'POST') {
        ...
        if (isset($_POST['imageId'], $_POST['imageName'], $_POST['imageDes'], $_POST['filePath'], $_POST['galleryList']) && !empty($_POST['galleryList'])) {

            $imageId = $_POST['imageId'];
            $imageName = validate_inputs($_POST['imageName']);
            $imageDes = validate_inputs($_POST['imageDes']);
            $filePath = $_POST['filePath'];
            $galleryList = $_POST['galleryList'];

            $filePath = $basePath . $filePath;

            $image_insert_query = "INSERT INTO images(image_id, image_name, image_path, image_description, image_gallery_id ) VALUES ('$imageId', '$imageName', '$filePath', '$imageDes', '$galleryList')";


            if ($conn->query($image_insert_query) == true) {
                echo "<script>show_alert('success-msg');</script>";
            } else {
                echo "<script>show_alert('danger-msg');</script>";
            }
        } else {
            echo "<script>show_alert('warning-msg');</script>";
        }
    }
