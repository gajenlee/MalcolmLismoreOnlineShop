...
<body class="theme-set disable-scroll">
        ...
        <div class="p-5 bg-body-tertiary mb-4" style="background-color: transparent !important;">
            ...
            <nav class="d-flex">
                <h6 class="mb-0">
                    ...
                    <span>/</span>
                    <a href="./update-shop.php" class="text-reset"><u>Shop</u></a>
                </h6>
            </nav>
        </div>
    ...
    <?php
    $item_id = $_GET["id"];
    $item_select_query = "SELECT * FROM items INNER JOIN images ON items.item_image_id = images.image_id WHERE item_id='$item_id'";
            ...
            <form action="" method="POST" class="myForm">
                <div class="shop-compands">
                    ...
                    <div class="form-outline" data-mdb-input-init>
                        <input name="itemName"  style="background-color: transparent;" class="form-control" id="itemName" type="text"  value="<?php echo $row["item_name"]; ?>" />
                        <label class="form-label" for="itemName">Item Name</label>
                    </div>
                    <div class="input-group mb-3">
                        <span class="input-group-text">$</span>
                        <input  type="text" name="price" id="price" style="background-color: transparent;" class="form-control" aria-label="Amount (to the nearest dollar)" value="<?php echo $row["item_price"]; ?>" />
                    </div>
                    <div class="form-outline" data-mdb-input-init>
                        <textarea class="form-control" name="itemDes" id="itemDes" rows="4"><?php echo $row["item_description"]; ?></textarea>
                        <label class="form-label" for="itemDes">Item Description</label>
                    </div>
                </div>
                <div class="image-compands">
                    ...
                    <div class="form-outline">
                        <select name="imageList" class="form-select" aria-label="Default select" style="background-color: transparent;">
                            <option selected>Select a Image</option>
                            <?php

                            $image_select_query = "SELECT * FROM images";
                            $result_images = mysqli_query($conn, $image_select_query);
                            while ($rows = mysqli_fetch_assoc($result_images)) {

                                $image_id = $rows["image_id"];
                                $image_name = $rows["image_name"];

                                if ($image_id == $row['image_id']) { 
                                    echo "<option selected value=$image_id>$image_name</option>";
                                } else {
    ...
    if (isset($_POST["submit"])) {
        ...
        if (isset($_POST['itemName'], $_POST['price'], $_POST['itemDes'], $_POST['imageList'])) {

            $item_name = validate_inputs($_POST['itemName']);
            $item_price = $_POST['price'];
            $item_des = $_POST['itemDes'];
            $item_image_id = $_POST['imageList'];


            $image_update_query = "UPDATE items SET item_name='$item_name',  item_description='$item_des', item_price='$item_price', item_image_id='$item_image_id' WHERE item_id='$item_id'";

            if($conn -> query($image_update_query) == true) echo "<script> window.location.href='./show-shop.php'; </script>";
